    ...
    try:
        ...
        for lang in preferred_languages:
            try:
                ...
                print(f"✅ Found manual transcript in: {manual.language_code}")
                return manual.fetch()
        ...
        for t in transcripts:
            if not t.is_generated:
                print(f"✅ Found manual transcript in: {t.language_code} (fallback)")
                return t.fetch()
        ...
        for lang in preferred_languages:
            try:
                ...
                print(f"✅ Found auto-generated transcript in: {auto.language_code}")
                return auto.fetch()
            except NoTranscriptFound:
                continue
        ...
        for t in transcripts:
            if t.is_generated:
                print(f"✅ Found auto-generated transcript in: {t.language_code} (final fallback)")
                return t.fetch()
...
def generate_keywords_and_summary(request):
    try:
        print(f"📨 Received {request.method} request to generate_keywords_and_summary")
        ...
        if request.method == 'POST':
            ...
            print(f"📝 POST data: url={url}, user_id={user_id}")
        elif request.method == 'GET':
            # Handle GET requests from frontend
            url = request.GET.get('url')
            user_id = request.GET.get('user_id')
            transcript = None  # No transcript in GET request
            print(f"📝 GET params: url={url}, user_id={user_id}")
        else:
            return JsonResponse({"error": "Method not allowed"}, status=405)

        if not url or not user_id:
            print(f"❌ Missing data: url={url}, user_id={user_id}")
        ...
        if not (title and channel_name and video_type):
            print(f"❌ Failed to get video information: title={title}, channel={channel_name}, type={video_type}")
            return JsonResponse({"error": "Failed to get video information"}, status=500)
        print(f"✅ Video details: {title} by {channel_name}")

        # Use provided transcript or try to fetch from backend using improved logic
        if not transcript:
            print("📝 No transcript provided, trying to fetch from backend...")
            try:
                transcript_data = get_youtube_transcript(video_id)
                ...
                print(f"❌ Backend transcript fetch failed: {e}")
                logger.error(f"Backend transcript fetch failed: {e}")
                return JsonResponse({"error": "No transcript available"}, status=400)
        else:
            print(f"✅ Using provided transcript: {len(transcript)} characters")

        # Generate summary and tags using improved video understanding
        try:
            print("🤖 Generating summary and tags with video understanding...")
